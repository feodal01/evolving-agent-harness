...
- `hypotheses/<id>.md`: full lifecycle dossier for each hypothesis, from proposal through result. Dossiers follow an **MCTS-style ledger**: **Search node (MCTS)** records parent state, rollout depth, value summary, deferred child actions, and revisit queue (see `docs/meta/prompt-orchestrator.md`).
