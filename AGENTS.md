...
Load OpenRouter credentials from local `.env` before benchmark work: `set -a; source .env; set +a`. Use `uv run evolve2 model-check` before benchmark work that depends on OpenRouter. Use `uv run evolve2 run-task --instance-id <id>` for one-task SWE-bench Verified runs.
